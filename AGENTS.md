...
- `CaptureRuleService` - runs `FloatyConfig.CaptureRules` ("capture Notepad when it opens", "capture devenv every 5 min"), editable from chat and Settings → Screen history. A 10 s poll of `ListWindowsAsync` (only while a rule is live), not a WinEvent hook, because rules must see background windows. On-open rules ignore windows already open when the rule starts. One capture per tick, at most one per 15 s overall; unchanged content is skipped via its own `CaptureDedupe`. Independent of `ScreenHistoryMode`; stored with source `rule`, so "Clear screen history" leaves them. Refuses `CaptureRedactor` excluded apps/private windows and redacts the text (the raw `CaptureWindowAsync` path doesn't).
