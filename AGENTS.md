...
- `ChatService` - gets its `IChatClient` from `AiClientFactory`; exposes the `search_captures` and `read_capture` AI tools, the `generate_image` / `edit_image` / `set_ring_image` tools when the image role is assigned, and, when the user scopes chat with `/server`, that MCP server's tools. Search hits show the passage that matched (`TextChunker.BestWindow`), not the opening; `read_capture` reads the rest from the saved file, resolved by name inside Floaty's own capture folders only.
