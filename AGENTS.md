...
- `ChatService` - gets its `IChatClient` from `AiClientFactory`; exposes the `search_captures` and `read_capture` AI tools, the `generate_image` / `edit_image` tools when the image role is assigned, the `notify` / `list_notifications` / `cancel_notification` tools where the platform can deliver notifications, and, when the user scopes chat with `/server`, that MCP server's tools. Everything else comes from `IChatToolset`s (`Services/Tools/`), registered in DI and folded in per turn with their guidance text. Search hits show the passage that matched (`TextChunker.BestWindow`), not the opening; `read_capture` reads the rest from the saved file, resolved by name inside Floaty's own capture folders only.
...
- `SkillService` - scans `~/.floaty/skills`, `~/.claude/skills`, `~/.agents/skills` for SKILL.md folders (YAML frontmatter + markdown body) and merges in the ones packaged under `app/Resources/Skills` (`generate-image`, `ring-image`); a folder of the same name on disk overrides a built-in. Invoked via `/name` slash commands in chat.
- `ImageGenerationService` - text-to-image through whichever provider holds the image role, over the OpenAI images endpoint (so OpenAI, Gemini's compatibility layer and any gateway alike). Writes PNGs to `~/.floaty/generated`; chat bubbles reference them as `floaty://image/<file>` rather than embedding base64, which is the only image URL the markdown renderer will load besides a `data:` URI.
...
- `Services/Tools/` - chat toolsets. `SettingsTools`: `get_settings`, `set_appearance`, `set_ring_image`, `set_sound`, `set_voice_output`, `get_system_prompt`, `update_system_prompt`; cosmetic only on purpose (keys, providers, exec, screen history, autostart and MCP stay Settings-only so a prompt injection can't widen what Floaty records or runs). `UpdateTools`: `check_for_updates`, `get_release_notes` (GitHub API, works in dev builds), `install_update`. `SystemTools` (over `ISystemIntegrationService`): `get_clipboard` (redacted via `CaptureRedactor`), `set_clipboard`, `open` (http/https/mailto, files, folders; refuses executables unless exec is enabled, and UNC paths always), `open_app` (Start Menu shortcuts only), `media_control`, `volume`, `list_windows`, `focus_window`, `system_info`. `CaptureTools`: `capture_window` (one-off, returns the text), `create_capture_rule` / `list_capture_rules` / `set_capture_rule_enabled` / `delete_capture_rule`. Gated tools (exec, `update_system_prompt`, `install_update`) ask through `ToolApproval` - an `AsyncLocal` callback the chat panel renders as its approval card; no callback means refuse. Tool writes go through `SettingsService.Save`, so any setting a tool can change must also be adopted in `SettingsViewModel.AdoptExternalState`, or an open Settings window's Save reverts it.
- `CaptureRuleService` - runs `FloatyConfig.CaptureRules` ("capture Notepad when it opens", "... 30 s after it opens", "capture devenv every 5 min"), editable from chat and Settings → Screen history. A 10 s poll of `ListWindowsAsync` (only while a rule is live), not a WinEvent hook, because rules must see background windows. On-open and after-open rules ignore windows already open when the rule starts (their open time is unknown); the after-open delay is in seconds, measured from first sight, so up to one poll late. One capture per tick, at most one per 15 s overall; unchanged content is skipped via its own `CaptureDedupe`. Independent of `ScreenHistoryMode`; stored with source `rule`, so "Clear screen history" leaves them. Refuses `CaptureRedactor` excluded apps/private windows and redacts the text (the raw `CaptureWindowAsync` path doesn't).
- `INotificationService` / `WindowsNotificationService` - native OS notifications behind the chat's `notify` / `list_notifications` / `cancel_notification` tools. **Scheduling is the OS's, not Floaty's**: a future notification is handed to Windows as a `ScheduledToastNotification`, so a reminder fires even after the user quits - there is no timer and nothing persisted under `~/.floaty`. Because Floaty is unpackaged, toasts only work through an AppUserModelID backed by a Start Menu shortcut; Velopack's installer already writes `velopack.Floaty` onto `Floaty.lnk`, so the service *adopts* that shortcut (never retargeting it - it points at `current\Floaty.exe`, which is what survives updates) and only creates one itself on dev or portable runs, lazily on the first notification. Ids are capped at 16 characters by WinRT.
...
All user data is local-first under the home directory: `config.json`, `floaty.md` (user system prompt), `floaty.db`, `captures/`, `conversations/` (one JSON per thread), `generated/` (images from the chat image tools), `skills/`, `models/` (with `models/embed/` for on-device embedding models), `native/`, `ring/`, `sounds/`. Never hardcode these paths - go through `FloatyPaths`.
