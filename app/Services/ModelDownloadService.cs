...
/// <see cref="FloatyPaths.SttModels"/>/&lt;model id&gt;. Whether a model is "downloaded" is always
/// derived from the expected files being present on disk — nothing is persisted in config.
/// </summary>
public sealed class ModelDownloadService
{
    ...
    /// <summary>True when every file of the model (and the shared Silero VAD) is on disk.</summary>
    public bool IsDownloaded(SttModelInfo model) =>
        model.IsAvailable && HasAllFiles(model) && HasAllFiles(SttModelCatalog.SileroVad);
    ...
    /// <summary>
    /// Downloads all files of <paramref name="model"/> (plus the Silero VAD dependency), reporting
    /// aggregate progress in [0,1]. Files stream to a ".partial" name and are renamed on completion,
    /// so a killed download never leaves a truncated file that passes <see cref="IsDownloaded"/>.
    /// </summary>
    public async Task DownloadAsync(SttModelInfo model, IProgress<double>? progress, CancellationToken ct = default)
    {
        ...
        var knownTotal = sizes.All(s => s.HasValue) ? sizes.Sum(s => s!.Value) : 0;
    ...
    /// <summary>Removes the model's folder (the shared Silero VAD is kept for other models).</summary>
